 ...
 */
window.clientTools.getCustomerName = async ({ spoken_customer_id }) => {
  const db = await loadDB();
  const id = normalizeDigits(spoken_customer_id);
  const rec = db.customers?.[id];
  if (!rec) return { ok: false, error: "customer_not_found", customer_id: id };
